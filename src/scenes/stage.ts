...
import { SceneContextSceneClass, SceneSessionData } from './context';
...
export class Stage<TContext = any> {
  ...
  middleware() {
    return async (ctx: any, next: () => Promise<void>) => {
      ...
      const scene = new SceneContextSceneClass<any, SceneSessionData>(
        ctx,
        this.scenes,
        {
      ...
      if (currentScene) {
        handled = await currentScene.handleUpdate(ctx as any);
        if (handled) {
          return;
        }
      }
